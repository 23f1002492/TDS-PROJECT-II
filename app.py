...
def parse_keys_and_types(raw_questions: str):
    ...
    Returns:
        ...
        has_keys: boolean indicating if keys were found
    """
    import re
    pattern = r"-\s*`([^`]+)`\s*:\s*(\w+)"
    ...
    has_keys = len(matches) > 0
    return keys_list, type_map, has_keys
...
async def analyze_data(request: Request):
    try:
        ...
        keys_list, type_map, has_keys = parse_keys_and_types(raw_questions)
        ...
        # Debug logging
        logger.info(f"DEBUG: dataset_uploaded={dataset_uploaded}, has_keys={has_keys}")
        logger.info(f"DEBUG: keys_list={keys_list}")
        logger.info(f"DEBUG: result before formatting: {type(result)}")

        # Determine output format based on presence of data file and keys
        if dataset_uploaded and has_keys:
            logger.info("DEBUG: Using Case 1 - Data file + keys -> object format")
            # Case 1: Data file uploaded + keys defined -> return object with key-value pairs
        ...
        elif not dataset_uploaded:
            logger.info("DEBUG: Using Case 2 - No data file -> array format")
            # Case 2: No data file -> return array of values
            result_array = []
            questions_in_order = list(result.keys())
            logger.info(f"DEBUG: questions_in_order: {questions_in_order}")
            for q in questions_in_order:
                val = result[q]
                if isinstance(val, list) and len(val) == 1:
                    val = val[0]
                elif isinstance(val, list) and len(val) > 1:
                    # If multiple items, join them or take first - depends on your needs
                    val = val[0]  # or ' '.join(map(str, val)) if you want to join
                result_array.append(val)
                logger.info(f"DEBUG: Added to array: {type(val)} - {str(val)[:100]}")
            result = result_array
            logger.info(f"DEBUG: Final array length: {len(result_array)}")
            
        else:
            logger.info("DEBUG: Using Case 3 - Data uploaded but no keys -> dictionary format")

        logger.info(f"DEBUG: Final result type: {type(result)}")
        logger.info(f"DEBUG: Final result: {str(result)[:300]}")
